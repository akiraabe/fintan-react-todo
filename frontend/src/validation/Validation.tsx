...
const useValidation = <T extends AvailableFiledType>(
  schema: FieldConstraintSchema<T>
) => {
  const [error, setError] = useState<ValidationError<T>>(
...
class ConstraintValidators<T extends AvailableFiledType> {
  ...
  constructor(
    ...
    for (const key of Object.keys(schema)) {
      ...
      init[key as keyof T] = new ConstraintValidator<any>(
        key,
        schemaItem,
        setValidationError
      );
  ...
  validateAll(value: T): boolean {
    ...
    for (const key of Object.keys(this.constraint)) {
      const error = this.constraint[key as keyof T].validate(
